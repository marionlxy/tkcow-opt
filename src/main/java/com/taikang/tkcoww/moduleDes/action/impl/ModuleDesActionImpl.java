...
public class ModuleDesActionImpl extends BaseActionImpl 
  implements IModuleDesAction {
	...
	@Override
	public Dto findNewOne(Dto paramDes) {
		logger.debug("<======ModuleDesAction--findNewOne======>");
		
		ModuleDesBO moduleDesBO = BaseDto.toModel(ModuleDesBO.class , paramDes);
		return moduleDesService.findNewOne(moduleDesBO).toDto();//返回的BO对象自动转换成Dto返回
	}
	
	/**
     * 查询单条数据
     */
	public Dto findOneModDes(Dto param) {
		logger.debug("<======ModuleDesAction--findOneModuleDes======>");
		
		ModuleDesBO moduleDesBO = BaseDto.toModel(ModuleDesBO.class , param);
		return moduleDesService.findOneModDes(moduleDesBO).toDto();//返回的BO对象自动转换成Dto返回
	}  
	...
	public CurrentPage getWithoutImgListModuleList(CurrentPage currentPage){
		logger.debug("<======ModuleDesAction--getWithoutImgListModuleList======>");
		
		return moduleDesService.getWithoutImgListModuleList(currentPage);
	}	
	...
	public CurrentPage getDevelopCourseModuleList(CurrentPage currentPage) {
		logger.debug("<======ModuleDesAction--getDevelopCourseModuleList======>");
		return moduleDesService.getDevelopCourseModuleList(currentPage);
	}
	...
	public CurrentPage getImgListModuleList(CurrentPage currentPage){
		...
		return moduleDesService.getImgListModuleList(currentPage);
	}

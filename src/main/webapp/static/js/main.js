...
wrapper.addTabCloseOld = function (subtitle, url, icon, oldTitle) {
    if (!url|| url == '#') return false;
    var $tab = $('#tabs');
    var tabCount = $tab.tabs('tabs').length;
    var hasTab = $tab.tabs('exists', subtitle);
...
//取得当前tab的title.
wrapper.getCurrentTabTitle = function () {
	var $tab = $('#tabs');
	var currentTab = $tab.tabs('getSelected');
	var titles = currentTab.panel('options').title;
	return titles;
}
...
//关闭以前存在旧菜单
wrapper.colseExistTitle=function (existTitle) {
	 var $tab = $('#tabs');
	    var titles = wrapper.getTabTitles($tab);
	    $.each(titles,function(idx){
	    	if(this==existTitle){
	    		 $tab.tabs('close', existTitle);
	    	}
	    		})
};
...
//合并两个json对象
function addJSONToJSON (json1, json2) {
	alert(JSON.stringify(json1));
	alert(JSON.stringify(json2));
	var result;
	alert((JSON.stringify(json1)+JSON.stringify(json2)).replace(/}{/,','));
	result = eval('('+(JSON.stringify(json1)+JSON.stringify(json2)).replace(/}{/,',')+')');
	return result
}

wrapper.tabCloseOne = function () {
    if (confirm('确认要关闭当前窗口吗？')) 
        wrapper.rightMenuClick({ id: 'close' });
};

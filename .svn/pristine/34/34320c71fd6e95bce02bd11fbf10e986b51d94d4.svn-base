...
//							pictureUrl.append(url+"/" +upPath+"/"+fileName + ",");
									pictureUrl.append(basePath+"/"+fileName + ",");
									System.out.println("下载路径 （存到数据库字段中的值） ："+pictureUrl);
								} catch (IllegalStateException e) {
									throw new TKCheckedException("文件上传出错：文件状态错误！");
								} catch (IOException e) {
	...
	public String uploadEditor(HttpServletRequest request,
			String upLoadPath, String string2) throws TKCheckedException {
			...
			while (iter.hasNext()) {
				...
				if (file != null) {
					...
					if (myFileName.trim() != "") {
						...
						// 重命名上传后的文件名
						String fileName = file.getOriginalFilename();
							fileName =  UUID.randomUUID().toString().replace("-", "")+fileName.substring(fileName.lastIndexOf("."),fileName.length());
						File fileDir = new File(upLoadPath);
						if (!fileDir.exists()) {
							if(!fileDir.mkdirs()) {
								throw new TKCheckedException("文件上传出错：文件路径配置错误！");
						...
						try {
							...
							file.transferTo(localFile);
							pictureUrl.append(upPath+"/"+fileName + ",");
						} catch (IllegalStateException e) {
							throw new TKCheckedException("文件上传出错：文件状态错误！");
						} catch (IOException e) {
							throw new TKCheckedException("文件上传出错：文件流读取出错！");

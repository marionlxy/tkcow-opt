 ...
 implements IRoleService<RoleBO>  
  {
	...
	public void deleteObject(RoleBO role) {
		...
		appDao.delete("Role.deleteBscRole",role);
	}
	...
	public List<Dto> getUsersRoleList(Dto param) {
		...
		if (roleList != null && roleList.size() > 0) {
			for (int i = 0; i < roleList.size(); i++) {
				...
				Integer roleId = dto.getAsInteger("id");
	...
	public void insertBscRole (RoleBO role) {
		logger.debug("<======RoleServiceImpl--insertBscRole======>");
		appDao.insert("Role.addOldRole",role);
	}
 }

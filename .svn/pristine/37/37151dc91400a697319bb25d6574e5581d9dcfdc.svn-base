...
public class CooperLogoController extends BaseController {
	...
	@ResponseBody
	public Map<String, Object> getTypeCooperLogoList(HttpServletRequest request, CurrentPage page) {
		Map<String, Object> map = new HashMap<String, Object>();

		page.setParamObject(CommonUtil.getParamAsDto(request));
		CurrentPage currentPage = cooperLogoAction.getTypeCooperLogoList(page);
	...
	private void saveCooperLogoInfo(HttpServletRequest request, HttpServletResponse response) {
		...
		Dto upload = CommonUtil.generateFile2Server("logoFile", path, request);
		if (StringUtils.isBlank(param.getAsString("cooperLogoId"))) {
			if (upload == null || upload.isAllEmpty() || !"0".equals(upload.getAsString("code"))) {
				result.put("code", "1");
			} else {
				param.put("cooperLogoImgname", upload.getAsString("originName"));
				param.put("cooperLogoUrl", upload.getAsString("storePath"));
				param.put("cooperLogoStorname", upload.getAsString("trueName"));

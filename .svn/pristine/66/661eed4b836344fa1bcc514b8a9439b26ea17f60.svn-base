...
<body>
	<div class="" title="logo模板·修改页面" style="padding: 10px 60px 20px 60px">
		<form id="dataFormLogoModelEdit" method="post"
			...
			<div class="fitem">
				<label>banner图片:</label> <input name="modBanner"	 class="easyui-filebox validatebox"  validType="img_upload"
					data-options="prompt:'Choose a image...'"
					style="width: 25%" /> <img src="<%=basePath%>/${modBannerUrl }"
					style="width: 50px; height: 50px;" />
			</div>
			<div class="fitem">
				<label>图标:</label> <input name="imgIcon"	class="easyui-filebox validatebox" validType="img_upload"
					data-options="prompt:'Choose a image...'"
					 style="width: 25%"/> <img src="<%=basePath%>/${modIco }" 
					 style="width:50px;height:50px;" />
			</div>
		</form>
